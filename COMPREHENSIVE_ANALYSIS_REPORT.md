...
### "평균 93.7% 정확도"의 함정 (Bit Accuracy Illusion)
- 워터마크 메시지가 16비트일 때, 15비트를 맞추면 수치상으로는 93.7%의 높은 Bit Accuracy를 기록합니다. 
- 하지만 딥페이크 출처 추적이나 저작권 증명 등 실제 응용 환경에서는 단 1비트만 틀려도 다른 사용자의 코드북과 동점(Tie)이 발생하거나 오인식(False Accept)될 수 있습니다. 
- 즉, 평균 비트 정확도는 높으나 16비트를 완벽하게 맞춘 **Exact-message Accuracy**는 강한 공격 시 절반 이하로 곤두박질치는 현상이 발생합니다.
